...
export class LockManager {
  ...
  private setEntryMetadata(entry: ResourceLockEntry, metadata: Record<string, unknown>): void {
    if (entry.type === 'skills') {
      ...
      if (m.skillFolderHash) e.skillFolderHash = m.skillFolderHash as string;
      if (m.categories) e.categories = m.categories as string[];
      if (m.namingStrategy) e.namingStrategy = m.namingStrategy as NamingStrategy;
      if (m.originalName) e.originalName = m.originalName as string;
      if (m.installedName) e.installedName = m.installedName as string;
      if (m.sourcePath) e.sourcePath = m.sourcePath as string;
    } else if (entry.type === 'subagents') {
      const e = entry as SubagentLockEntry;
      const m = metadata as Record<string, unknown>;
      if (m.model) e.model = m.model as string;
      if (m.format)
        e.format = m.format as
          | 'coding-agent-fabric-json'
          | 'claude-code-yaml'
          | 'markdown-frontmatter';
      if (m.configHash) e.configHash = m.configHash as string;
    } else {
